---
name: export-report
description: "Genera un informe funcional ejecutivo (`.md`) consolidando el corpus de sesiones del workspace y `docs/`, dirigido a gerencia/jefatura/comité. Variante B default (≤760 palabras v1.1), A (400w compacto) y C (1620w extenso v1.1) derivadas vía `--audiencia`. Output en `docs/funcional/NNN-export-report-YYYY-MM-DD.md`. Read-only sobre el corpus; sin commits autónomos. Aplica una tabla determinista de traducción técnico→ejecutiva y valida cota + léxico vetado post-generación. Invocado sólo vía `/agent-workflow:export-report`. Renombrado desde `export-func` (session057) → `export-functional-specs` (session059) → `export-functional-report` (session060) → `export-report` (session081, nombre corto canónico, sin alias legacy). v1.1 (session076): renombró Finalidades→Objetivo y Recomendaciones→Oportunidades de mejora, agregó Componentes impactados + Diagrama de flujo a B/C. v1.2 (session077): el diagrama de flujo pasa a Mermaid `flowchart LR` por default. v1.3 (session078): link `https://mermaid.ink/img/<base64>` debajo de cada bloque Mermaid. v1.4 (session079): matriz `--audiencia × --mode` actualizada a cotas v1.1, rebalance pesos A + tolerancia condicional C. v1.7 (session081): rename a `export-report` (hard, sin alias legacy — DEC-003) + corpus extendido a `docs/` además de sesiones (DEC-002) — ver `docs/shared-contract/export-corpus-sources.md`."
version: 1.7.0
---
