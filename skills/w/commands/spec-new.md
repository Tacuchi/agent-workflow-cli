...
3. **Slug and draft** — derive `<slug>` from the Requirement: short kebab-case, only `[a-z0-9-]`, ≤ ~5 words / ≤ 40 chars. Then frame `$ARGUMENTS` into the draft schema below; reconnaissance findings land **only** where the filling notes allow.
4. **Publish it in one act** — `aw next-number docs/specs --publish spec-<slug>.md` with the finished draft on stdin: it assigns the correlative and writes the document atomically, and returns `published_path`. This command has no session, so it never leaves a reservation of nobody's — and an interruption leaves neither file nor consumed number. On an accepted split: repeat 3-4 per part.
