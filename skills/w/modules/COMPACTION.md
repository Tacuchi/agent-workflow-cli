...
> **Which mode runs, and whether the host can honour it, is not this document's call:** `aw checkpoint-write` decides it from the `[compaction]` config (`mode` = `confirm` | `auto`), the host's binding and the session's state. `auto` needs a **non-interactive** mechanism and **degrades to `confirm`** without one; **CHECKPOINT before compacting** holds in every mode, and an ambiguity no longer retains the compaction: it degrades and parks a refuge checkpoint to adopt later. What this document keeps is why: consent is never skipped where a person can be asked, and a compaction that fires before the checkpoint loses the thread it was meant to protect.
