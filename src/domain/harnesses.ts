...
export const HARNESSES: readonly HarnessSpec[] = [
  ...
  },
  {
    ...
    structuredChoice: {
      ...
      customAnswer: true,
      fallbackReason:
        "the turn does not list it — Default mode leaves it out and `codex exec` never supports it; enabling the `default_mode_request_user_input` opt-in does NOT add it (probed 2026-08-22), so switching mode in the TUI (`/plan`, `/pair`) is the person's move to offer, never the agent's to make",
      evidence:
